...
def _train_agent_core(
    agent_index: int,
    round_idx: int,
    agents: List[PPOAgent], # Currently focused on PPOAgent, but could receive BaseAgent if they supported train()
...
def train_single_agent_worker(
    ...
    env_factory: Callable[[int, List[BaseAgent], int], BaseMARLEnv], # BEWARE: pickled callable must be importable or simple
    timesteps_per_agent: int,
    save_dir: Optional[str],
    verbose: bool,
    N: int,
